...
class ConfirmedNewHires:
    """Class to verify which new hires are needing emails this week"""

    def confirm_letters(self, old_letters):
        """Function to manually confirm which new hires are starting based on a list of users"""
        ...
        while True:
            ...
            for i in numbers:
                ...
                # Make sure all the numbers are within range and remove a number if it is
    ...
    @staticmethod
    def auto_confirm():
        """Method to pull the new hires from a generated CSV"""
        next_week = "csv for confirmed new hires from SNOW"
        confirmed = []

        # Pull the confirmed users from the csv
        data = pd.read_csv(next_week)
    ...
    def print_list(users):
        """Simple method to print the users to the console"""
